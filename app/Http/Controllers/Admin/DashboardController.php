...
class DashboardController extends Controller
{
    public function index()
    {
        ...
        $districts = Location::withCount('facilities')
            ...
            ->get()
            ->groupBy('district')
            ->map(function ($locations, $district) {
                $totalLocations = $locations->count();
                $totalFacilities = $locations->sum('facilities_count');

                $totalRevenue = $locations->flatMap(function ($location) {
        ...
        $total_Revenue_query = Booking::where('status', 'confirmed')
        ->get();

        $totalRevenue = $total_Revenue_query->sum('total');


        $totalPending = $pendingBookingsByUser->sum('total_pending');
        $totalBookingCount = Booking::whereIn('status', ['approved', 'confirmed'])->count();
